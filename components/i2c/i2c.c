...
esp_err_t i2c_master_init(i2c_master_bus_handle_t *bus_handle)
{
    i2c_master_bus_config_t bus_config = {
        ...
        // Helps reject short glitches on the line (useful with longer wires/connectors)
        .glitch_ignore_cnt = 7,

        // Use synchronous transactions (no async queue needed here)
        .trans_queue_depth = 0,
    ...
    ESP_LOGI(TAG, "I2C master initialized successfully (SDA=%d, SCL=%d, %d Hz)",
...
esp_err_t i2c_write(i2c_master_dev_handle_t dev_handle, const uint8_t *data, size_t data_len)
{
    ...
    ESP_LOGD(TAG, "Wrote %u bytes to I2C device", (unsigned)data_len);
    return ESP_OK;
}
...
esp_err_t i2c_read(i2c_master_dev_handle_t dev_handle, uint8_t *data, size_t data_len)
{
    ...
    ESP_LOGD(TAG, "Read %u bytes from I2C device", (unsigned)data_len);
    return ESP_OK;
}

// Write data to an I2C device, then read data back in a single transaction
esp_err_t i2c_write_read(i2c_master_dev_handle_t dev_handle,
                         ...
                         uint8_t *rd, size_t rd_len)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to write+read I2C device: %s", esp_err_to_name(err));
    }
    return err;
}
